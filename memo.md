...
git clone でディレクトリを作りたくない場合は.を付けるとよい。
...
例えば、git merge origin/master というコマンドは origin(GitHub)の master ブランチの内容を自分のブランチにマージする。
...
ポインタを前に進めるだけで、新たなコミットは発生しない。

- Auto Merge:基本的なマージ

master ブランチと feature ブランチが枝分かれしている場

master ブランチにいる状態で、git merge feature とコマンドを打つと、master ブランチに feature ブランチの内容がマージされた新しいコミットが作成される。

このように作成されたコミットは親のコミットが 2 つ存在する点に注意。

- Conflict:コンフリクト

複数人が同じファイルの同じ箇所に変更を行った場合に発生する。コンフリクトが発生したら、慌てずにファイルを修正してから新たにコミットを行う。

コンフリクトを防ぐ為の運用ルール

- 複数人で同じファイルを変更しない
- pull や merge を行う前に、変更中の状態をなくしておく
- pull する際は、pull を行うブランチに移動してから pull を行う

### ローカルを GitHub に合わせて綺麗にする方法例

master ブランチを削除して、 GitHub の master ブランチと合わせる

> git branch -D master

> git fetch

git branch <新規に作成するブランチ名> <ベースとなるブランチ名>

> git branch master origin/master

### ブランチの変更、削除

-m は move、-d は delete の略。

自分が作業しているブランチの名前を変更する。

> git branch -m <ブランチ名>

もし、削除するブランチにマージされていない変更がある場合には、警告を出して削除は行わない。

> git branch -d <ブランチ名>

マージされていない変更があっても強制的に削除する場合

> git branch -D <ブランチ名>

### リモートブランチ

リモートブランチは<リモート名>/<ブランチ名>で参照できる。

リモートレポジトリ

a62324← コミット 1(master)
← コミット 2(feature)

ローカルレポジトリ

a62324(master)← コミット 3(master)
↓
git fetch を実行し、リモートレポジトリのコミットファイルの情報を取得。(origin/ブランチ名)
↓
a62324(master)← コミット 3(master)
← コミット 1(origin/master)
← コミット 2(origin/feature)
...
https://github.com/kyamasan/github_theory/pulls
↓
New Pull Request ボタンをクリック
↓
base:master ← compare:pull_request を選択
↓
Create Pull Request ボタンをクリック ×2
↓
Reviewer でレビューしてほしい人を選択。
↓
レビュワーは
https://github.com/kyamasan/github_theory/pulls
の file changed タブからファイルの変更を確認。
(＋ボタンでコメント可能)
↓
Confirm Merge ボタン、Delete branch ボタンをクリックで完了。
